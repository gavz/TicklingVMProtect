...
PointerType SegmentsAAResult::getPointerType(const llvm::Value *V, std::set<const llvm::Value *> &visited) const {
  ...
  auto getIndirectPointerType = [&](const llvm::Value *B) {
    if (const auto *A = llvm::dyn_cast<llvm::Argument>(B)) {
      ...
      auto PtrTy = getPointerType(B, visited);
      switch (PtrTy) {
